...
export default function StudentShell({ name, avatarUrl, children }: { name: string; avatarUrl: string|null; children: React.ReactNode }) {
  return <div className={styles.shell}>
    <aside className={styles.sidebar}><Link className={styles.brand} href="/student" mode="auto" showPendingLabel={false} lockWhilePending={false}><i>N</i><span><strong>NSP</strong><small>Кабинет ученика</small></span></Link><StudentNavigation/><footer><StudentAccountMenu name={name} avatarUrl={avatarUrl}/></footer></aside>
