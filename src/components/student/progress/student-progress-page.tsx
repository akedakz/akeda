...
export default function StudentProgressPage({ data, mode = "student", studentId, showHeader = true }: { data: StudentProgressData; mode?: "student" | "admin" | "parent"; studentId?: string; showHeader?: boolean }) {
  const [modal, setModal] = useState<ModalName>(null);
  const closeModal = useCallback(() => setModal(null), []);
  return <div className={styles.page}>
    {showHeader && <header className={styles.heading}><span>{mode === "student" ? "Личный кабинет" : "Результаты ученика"}</span><h1>{mode === "student" ? "Мой прогресс" : "Прогресс"}</h1><p>{mode === "student" ? "Ваши темы, уроки, домашние задания, тесты и тренажёры" : "Темы, уроки, домашние задания, тесты и тренажёры"}</p></header>}
    ...
    <StudentProgressModal title="Все тесты" open={modal === "tests"} onClose={closeModal}><div className={styles.modalList}>{data.tests.items.length ? data.tests.items.map((item) => { const content = <><div><strong>{item.title}</strong>{item.dateLabel && <time>{item.dateLabel}</time>}</div><b>{item.status === "COMPLETED" ? `${Math.round(item.percent ?? 0)}%` : item.status === "MISSED" ? "Пропущен" : "Не завершён"}</b></>; const href = item.attemptId ? mode === "admin" && studentId ? `/admin/students/${studentId}/tests/${item.assignmentId}/attempts/${item.attemptId}` : mode === "student" ? `/student/tests/${item.assignmentId}/attempts/${item.attemptId}` : null : null; return href ? <Link className={styles.testRow} href={href} key={item.assignmentId}>{content}</Link> : <article className={styles.testRow} key={item.assignmentId}>{content}</article>; }) : <p className={styles.modalEmpty}>Тесты пока не назначены.</p>}</div></StudentProgressModal>
